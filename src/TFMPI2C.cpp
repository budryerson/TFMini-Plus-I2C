 ...
 * v.1.6.0 - 06SEP21 - Corrected (reversed) Enable/Disable commands in 'TFMPI2C.h'
 * v.1.7.0 - 03NOV21 - No longer need pin numbers for 'recoverI2C()' routine.
...
void TFMPI2C::recoverI2CBus( uint8_t dataPin, uint8_t clockPin)
{
    ...
    pinMode( dataPin, OUTPUT);     //keep SDA high during recovery
...
//
//  Recover bus without specifying the pin numbers
//  that are defined in 'variants.h' for each board.
void TFMPI2C::recoverI2CBus()
{
    recoverI2CBus( PIN_WIRE_SDA, PIN_WIRE_SCL);
    // The Arduino Due has a second I2C interface, so...
