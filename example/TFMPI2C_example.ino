...
#include <TFMPI2C.h>  // TFMini-Plus I2C Library v0.1.0
TFMPI2C tfmP;         // Create a TFMini-Plus I2C object

void setup()
{
    Wire.begin();            // Initialize two-wire interface
//    Wire.setClock( 400000);  // Set I2C clock speed to Fast if
                             // your Arduino can support 400KHz.
    ...
    if( tfmP.sendCommand( OBTAIN_FIRMWARE_VERSION, 0))
    {
        ...
        printf( "%1u.", tfmP.version[ 1]); // separated by dots
        printf( "%1u\r\n", tfmP.version[ 2]);
    }
    else tfmP.printErrorStatus();
    // - - - - - - - - - - - - - - - - - - - - - - - -
